...
- 対象ドキュメントは、変更対象のドメインに対応する設計図です。少なくとも共通設計として 、関連するドメインのMMDファイル、DB設計の変更を伴う場合は `docs/db/01_ER図.mmd` を確認してください。
- クエスト関連の設計変更では、`docs/quest/04_クエストドメイン.mmd` を優先して更新してください。
- GitHub Actions などから起動するメンテナンス・クリーンアップ処理の変更では、`docs/maintenance/01_クリーンアップドメイン.mmd` も更新してください。
