...
* 戦士職と盾兵職の「ボスかどうか」「防御力が自身の攻撃力の 70% 以上かどうか」の判定は、この攻撃対象として選ばれた敵を基準に行う。
* 対象敵の「防御力が自身の攻撃力の 70% 以上」は `targetDefense >= actorAttack * 0.7` で判定する。
...
* 「MP 回復技」は将来的に `Move` から判定できるようにする想定だが、現時点の `MoveDomain` / `BattleDomain` には MP 回復効果の表現が存在しない。
* これらの技種別判定は生 CSV を直接参照せず、`IMoveRepository` が組み立てた `Move` オブジェクトの `TargetType`, `AttackRange`, `Effects` から導出する。
* そのため MP 回復技を味方 NPC ルールで使用するには、後続で `MoveEffectType` などへ MP 回復効果を追加する必要がある。
...
* 対象敵の防御力が自身の攻撃力の 70% 以上であり、使用可能な単体攻撃技がある場合は、その技を使用する。
* ボス条件と高防御条件が同時に成り立つ場合も、選択結果は「使用可能な単体攻撃技を使う」で同じとみなす。
...
* MP 温存条件に入っている間は、ちょうはつ再付与に必要な MP を守るため、MP 消費技を新たに選択しない。
* 対象敵がボスであり、防御参照技があり、かつ MP 温存条件に入っていない場合は、防御参照技を使用する。
* 対象敵の防御力が自身の攻撃力の 70% 以上であり、防御参照技があり、かつ MP 温存条件に入っていない場合は、防御参照技を使用する。
...
* ただしこの MP 回復技ルールは先行して要件化するものであり、現時点の `MoveDomain` / `BattleDomain` では MP 回復効果を表現できないため、実装前にドメイン拡張が必要である。
...
* `祈り` は既存の通常攻撃や `UseMove` とは別の行動種別として追加する想定であり、現時点の `QuestDomain` / `BattleDomain` では未定義である。
* そのため僧侶職の `祈り` ルールを実装するには、後続で行動種別と解決ロジックを拡張する必要がある。
...
#### 味方 NPC 行動ロジックに関する未実装項目

味方 NPC の職業別行動方針は本節で先行して要件化したが、現時点では以下の未実装項目が残る。

`MoveDomain` の拡張が必要な項目:

* MP 回復技
  現行の `MoveEffectType` では MP 回復効果を表現できないため、魔法職の MP 枯渇時ルールを実装するには効果種別または同等の表現追加が必要である。
* 技メタデータ不足時の補助情報
...
* `祈り`
  僧侶職の `祈り` は既存の通常攻撃や `UseMove` と別の行動種別を前提としており、現行の行動解決モデルでは未定義である。
* MP 回復効果の解決
  MP 回復技を導入する場合、`CurrentMp` を回復する行動結果を戦闘解決へ組み込む必要がある。

`QuestDomain` / クエスト進行側で追加実装が必要な項目:

* `QuestAllyNpcActionPolicy`
  味方 NPC の職業別ルールに従って毎ターンの行動を選択する専用ポリシーは、まだ実装されていない。
* 味方 NPC 行動生成の差し替え
  現在の単純な自動通常攻撃生成を、`QuestAllyNpcActionPolicy` を使う形へ差し替える必要がある。
* 罠の設置状態判定
  レンジャー職の「罠が未設置なら罠設置技を優先する」を実装するには、そのターン開始時点で有効な罠が存在するかを判定するロジックが必要である。
* 僧侶職の回復量評価
  「使用後 HP が 90% から 100% になる回復技を優先する」判定を行うため、回復技ごとの見込み回復量を比較するロジックが必要である。
