...
* スキルマスタは既存の `MoveDomain.Move` を利用し、`move_id` は既存 CSV マスタを参照する。
...
* `BattleActionResult` はログ表示と演出再生のため、`ActorId` に加えて `ActionKind` と `MoveId` を持つ。各 `BattleTargetResult` は行動単位の `HpChange` / `MpChange` を持ち、後続行動の影響を受けずにログ文言を生成できるようにする。失敗時は `FailureReason` を保持し、クエスト側で「行動前に倒れた」「対象がいなかった」などの定型文へ変換する。
* これによりクエスト側は、ターン解決後の各行動について「だれが」「何の行動を」「どのスキルで」行ったかを結果 DTO だけで把握できる。
* 単体攻撃や単体対象スキルの入力は「どのマスを狙ったか」を `BattlePosition` で保持し、解決時にその座標にいる対象へ適用する。
...
待機画面での表示用プロフィール情報は `QuestRoom` 自体には保持せず、ルーム詳細レスポンス生成時に `Player` から参照して投影する。QuestRoomLobbySectionProps
...
`GET /quest/rooms/{roomId}` の `participants` は待機画面描画のため、以下の表示用項目を追加する。
...
* `MoveSet` はクエスト開始時点で使用可能なスキルスロット構成を表す。詳細効果は `MoveDomain.Move` を参照し、クエスト固有の使用制約がある場合はスナップショット側で補助情報を持つ余地を残す。
...
* `QuestTurnCommand` は入力として保持するが、クライアントのログ表示やスキルエフェクト表示は、最終的には `BattleActionResult` に含まれる `ActorId` / `ActionKind` / `MoveId` を基準に行う。
...
* `availableMoves` は学習済みスキル一覧を指し、MP 残量などのそのターン条件を加味した使用可否判定は `QuestAllyNpcActionPolicy` 側で行う。
...
* 「防御上昇系統のスキル」は、`Move.Effects` に `Buff` 効果を含み、その `Buff.BuffStat == Defense` であるスキルとして判定する。
* 「状態異常スキル」は、`Move.TargetType == Enemy` であり、`Move.Effects` に `Ailment` 効果を含むスキルとして判定する。
* 「罠設置スキル」は、`Move.Effects` に `Ailment` 効果を含み、その `AilmentType == DamageTrap` または `AilmentType == PoisonTrap` であるスキルとして判定する。
* 「MP 回復スキル」は、`Move.Effects` に MP 回復効果を含むスキルとして判定する。
* これらのスキル種別判定は生 CSV を直接参照せず、`IMoveRepository` が組み立てた `Move` オブジェクトの `TargetType`, `AttackRange`, `Effects` から導出する。
* MP 回復スキルを扱うため、`MoveEffectType` と戦闘解決へ MP 回復効果を追加する。
* 条件に合致するスキルが複数ある場合は `MoveSet` のスロット順で先頭のものを採用する。
* どの条件にも合致するスキルが存在しない場合は通常攻撃を選ぶ。

戦士職:

* 基本行動は通常攻撃とする。
* 対象敵がボスであり、使用可能な単体攻撃スキルがある場合は、そのスキルを使用する。

盾兵職:

* 自身にちょうはつ状態が付与されていない場合は、まずちょうはつスキルの使用可否を判定する。
* ちょうはつ状態がなく、かつ MP 温存条件に入っていない場合は、ちょうはつスキルを最優先で使用する。
* MP 温存条件は `CurrentMp <= floor(MaxMp * 0.3)` で判定する。
* MP 温存条件に入っている間は、ちょうはつ再付与に必要な MP を守るため、ちょうはつ再付与を除く MP 消費スキルを新たに選択しない。
* MP 温存条件に入っていても、自身にちょうはつ状態がなく、かつちょうはつに必要な MP が残っている場合はちょうはつスキルを使用する。
* 対象敵がボスであり、防御参照スキルがあり、かつ MP 温存条件に入っていない場合は、防御参照スキルを使用する。
...
* 攻撃対象がボスであり、使用可能な単体攻撃スキルがある場合は、MP 残量に関係なく単体攻撃スキルを使用する。
* ボスに対する単体攻撃スキルの選択は、使用可能な単体攻撃スキルのうち MP 消費が最も少ないものを採用する。
* 攻撃対象がボスではなく、かつ MP が最大 MP の 50% 以上である場合は、使用可能な範囲攻撃スキルのうち最も攻撃範囲が広いものを使用する。
* 範囲攻撃スキルどうしの比較順は `All` > `Square` > `Row` / `Column` > `Single` とし、同順位なら MP 消費が高いものではなく `MoveSet` のスロット順が先のものを採用する。
* 攻撃対象がボスではなく、かつ MP が最大 MP の 50% を下回る場合は、使用可能な単体攻撃スキルのうち MP 消費が最も少ないものを使用する。
* 単体攻撃スキルの MP 消費が同値の場合は `MoveSet` のスロット順が先のものを採用する。
* 上記の攻撃スキルをいずれも使用できない MP 残量まで枯渇しており、かつ MP 回復スキルが定義済みである場合に限り、MP 回復スキルを使用候補に含める。
* 上記条件に合う攻撃スキルが存在しない場合は通常攻撃とする。
...
* 回復スキルの選択では、使用後 HP が 90% 以上 100% 以下になるスキルを優先する。
* 90% 以上 100% 以下に収まる回復スキルが複数ある場合は、回復後 HP が 100% に最も近いものを採用する。
* 90% 以上 100% 以下に収まる回復スキルが存在しない場合は、100% を超えない範囲で最も回復量が大きい回復スキルを採用する。
* それも存在しない場合は、回復後 HP が 100% を最も少なく超過する回復スキルを採用する。
* 回復対象が存在せず、防御上昇系統のスキルがある場合は、そのスキルを使用候補にする。
* 防御上昇系統のスキルどうしの比較順は、まず `AttackRange` の広さを優先し、同順位なら `Buff.BuffValue` が大きいものを優先する。
* 防御上昇系統のスキルの範囲比較順は `All` > `Square` > `Row` / `Column` > `Single` とする。
* 防御上昇系統のスキルでも同順位の場合は `MoveSet` のスロット順が先のものを採用する。
* 回復対象が存在せず、防御上昇系統のスキルもない場合は `祈り` を使用する。
...
* 有効な罠が未設置であり、使用可能な罠設置スキルがある場合は、そのスキルを最優先で使用する。
* 罠設置スキルが複数ある場合は、`MoveSet` のスロット順が先のものを採用する。
* 罠設置済み、または罠設置スキルを持たない場合は、攻撃対象候補のうち後衛または中衛の敵に対して状態異常スキルを優先する。
* 状態異常スキルの対象探索順は `後衛・左` -> `後衛・右` -> `中衛・左` -> `中衛・右` を優先し、該当対象がいない場合のみ前衛を見る。
* 攻撃対象がボスである場合でも状態異常スキルは候補に含めてよい。
* ただしボスに対して状態異常スキルを使う場合は、一律で状態異常適用確率を下げる補正を掛ける。
* 状態異常スキルを使用しない場合は、使用可能な単体攻撃スキルのうち MP 消費が最も少ないものを使用する。
* 単体攻撃スキルの MP 消費が同値の場合は `MoveSet` のスロット順が先のものを採用する。
...
* 初期実装では敵は支援スキルや自己強化スキルの能動使用を行わず、ダメージ系スキルと通常攻撃だけを選択対象とする。
...
* `QuestAllyNpcActionPolicy` 自体は `IMoveRepository` を持たず、スキル定義の解決はアプリケーション層が行う。
...
* スキルメタデータ不足時の補助情報
  現在は `TargetType`, `AttackRange`, `Effects` から単体攻撃スキル・ちょうはつスキル・防御参照スキル・範囲攻撃スキル・回復スキル・防御上昇スキル・状態異常スキル・罠設置スキルを判定する前提だが、今後それで識別できないスキル種別が出る場合は `MoveDomain` 側の設計補強が必要である。
...
* ボス相手状態異常の補正
  ボス階層に存在する敵へ状態異常スキルを使用する場合、一律の成功率補正を適用する。
...
* 味方 NPC 向けスキルセット整備
  各職業ルールを成立させるには、NPC テンプレートに対応するスキル構成が CSV 上で整っていることを必須とする。
...
* `祈り` は初回実装に含める。
* MP 回復スキルを扱うため、`MoveDomain` と `BattleDomain` を拡張する。
* クエスト中の範囲スキルは単体へ潰さず、プレイヤー手動入力と NPC 自動入力で同一の対象表現を用いる。
* ボス判定は敵個体単位ではなく、ボス階層に存在する敵全員をボスとして扱う。
* `ActionMode.AutoAttackOnly` はプレイヤー放置時の通常攻撃制限に使い、NPC については同モード中でも自動思考の例外として特殊行動を許可する。
* `availableMoves` は学習済みスキル一覧とし、MP 残量などのそのターン条件を加味した使用可否判定は `QuestAllyNpcActionPolicy` が担当する。
* レンジャーの罠再設置判定はパーティ全体で行い、そのターン開始時点で有効な罠が 1 つでもあれば再設置しない。
* レンジャーはボス相手にも状態異常スキルを使用候補に含めるが、状態異常の適用確率にはボス補正を掛ける。
...
#### スキルエフェクト画像は CSV オプション項目として扱う

スキルエフェクト画像は既存のスキル CSV における任意項目として扱い、DB テーブル追加は行わない。
クライアントは `EffectImagePath` が設定されているスキルだけ画像演出を表示し、未設定スキルは従来どおりテキストや既定演出のみで処理する。
行動解決結果には `MoveId` を含め、クライアントはその `MoveId` を使って `EffectImagePath` を参照する。

#### 行動解決結果はログ表示に必要な識別子を持つ

`BattleActionResult` は `ActorId` に加えて `ActionKind` と `MoveId` と `FailureReason` を持つ。`BattleTargetResult` は `HpChange` / `MpChange` を保持する。
これによりクエスト側は入力コマンドの再解釈に依存せず、解決順どおりの行動ログとスキル演出を構築できる。
