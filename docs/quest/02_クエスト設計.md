...
本ドキュメントは `docs/quest/01_クエスト機能定義.md` を、既存の `Player` / `Move` ドメインと `internal` スキーマ運用へ接続するための設計ドラフトである。
現時点では以下を目的とする。

* クエスト機能の責務境界を定義する。
* 永続化が必要な情報と、クエスト実行中だけ必要な情報を切り分ける。
* `docs/player/01_コアドメイン.mmd` と `docs/db/01_ER図.mmd` を更新する前段として、用語と状態遷移を固める。
...
1. `docs/player/01_コアドメイン.mmd`
2. `docs/db/01_ER図.mmd`
3. 必要なら `docs/common/01_Enum.mmd`
